...
class Formas{
    ...
    dibujar(){
        ...
        for(let i = 0; i < cantidadFig; i++) {     
         ...
         if(val==1){
          ...
          tint(red(this.colores1[i]),green(this.colores1[i]),blue(this.colores1[i]),this.opacidad);

         }else if(val==2){

          // this.colores2[i] = lerpColor(this.colorInicial2,this.colorFinal2,mezcla);
          tint(red(this.colores2[i]),green(this.colores2[i]),blue(this.colores2[i]),this.opacidad);

         }else if(val==3){
          // this.colores3[i] = lerpColor(this.colorInicial3,this.colorFinal3,mezcla);
          tint(red(this.colores3[i]),green(this.colores3[i]),blue(this.colores3[i]), this.opacidad);
         }
    ...
    anch(valor){
      for(let i = 0; i < cantidadFig; i++) {       
        push();               
        this.ancho[i]= map(valor,0,1,60,120);//int(random(30, 90));
        pop();
    ...
    alt(valor){
      for(let i = 0; i < cantidadFig; i++) {   
        push();                    
         this.alto[i]=map(valor,0,1,80,120); //int(random(80, 90));
         pop(); 
      }
    }
    sep(valor){
      for(let i = 0; i < cantidadFig; i++) {   
        push();                    
        this.separacion[i]=map(valor,0,1,30,75);  //int(random(30,60));  
        pop();
        // this.x= this.x + random(-5,5);
